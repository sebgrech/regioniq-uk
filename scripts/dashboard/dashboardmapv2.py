...
GEO_ITL1 = Path("data/geo/ITL1_simplified_clean.geojson")  # Correct GeoJSON path

# Fixed GeoJSON property key - will be detected automatically
GEO_ITL1_FEATURE_KEY = "properties.ITL121CD"  # Default, will be overridden by auto-detection
...
def load_geojson():
    ...
    if not GEO_ITL1.exists():
        st.error(f"GeoJSON file not found at: {GEO_ITL1.absolute()}")
        return None
    
    try:
        geojson = json.loads(GEO_ITL1.read_text())
        
        # Debug: Check GeoJSON structure
        if 'features' in geojson and len(geojson['features']) > 0:
            first_props = geojson['features'][0].get('properties', {})
            print(f"GeoJSON loaded successfully. Available property keys: {list(first_props.keys())}")
            
            # Try to find ITL code in properties
            itl_keys = [k for k in first_props.keys() if 'ITL' in k.upper()]
            if itl_keys:
                print(f"Found ITL-related keys: {itl_keys}")
